...
lower_text = list(map(lambda x: str.lower(x), split_list))
print (lower_text)


# получить из list пункта 3 dict, ключами которого являются слова, а значениями их количество появлений в тексте;
word_times_dict = dict()
for times in split_list:
    if times in word_times_dict.keys():
        word_times_dict[times]+=1
    else:
        word_times_dict[times]=1
...
word_list_sorted = list(word_times_dict.items())
word_list_sorted.sort(key=lambda word: word[1])
word_set = set(split_list)
print("Пять наиболее встречаемых слов:",*word_list_sorted[-5:])
print("Количество разных слов в тексте:",len(word_set))
